---
name: Order and Sales at Kosmetikatama Super Indah (Inez Cosmetics)
url: ""
images:
  - /images/portfolios/order-and-sales-at-kosmetikatama-super-indah-inez-cosmetics/1779968082705-neu03j.webp
stacks:
  - id: visual-basic-net
    name: Visual Basic .Net
    url: https://learn.microsoft.com/en-us/dotnet/visual-basic/
    logo: /images/stacks/visual-basic-net/1779968072875-j32lq.webp
    color: ""
    highlight: false
  - id: ms-sql-server
    name: MS SQL Server
    logo: /images/stacks/ms-sql-server/1779968072414-zvjmwh.webp
    color: ""
    highlight: false
    url: https://www.microsoft.com/en-us/sql-server/
project_type: Company
role: Backend
highlight: false
description: Developing a web service to receive and store data on orders and
  sales made by Beauty Advisors (BA) in various store locations. The system also
  records stock data and provides reports on transactions conducted by BAs
---

Developing a web service to receive and store data on orders and sales made by Beauty Advisors (BA) in various store locations. The system also records stock data and provides reports on transactions conducted by BAs
